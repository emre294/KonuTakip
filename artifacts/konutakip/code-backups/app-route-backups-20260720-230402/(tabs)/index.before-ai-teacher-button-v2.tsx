...
function CountdownCard({ title, date, color, colors }: {
  ...
  return (
    <View style={[styles.countdownCard, { backgroundColor: color }]}>
      ...
      <Text style={styles.countdownDayLabel}>GÃœN KALDI</Text>
    ...
    <View style={{ width: size, height: size, alignItems: "center", justifyContent: "center" }}>
      <Svg width={size} height={size} style={{ position: "absolute" }}>
        ...
        <Circle
          ...
          fill="none"
        />
        {/* Colored progress arc â€” dashoffset=circumference at 0% so nothing is visible */}
        <AnimatedCircle
          cx={size / 2}
          cy={size / 2}
          r={radius}
...
function ProgressCard({ title, pct, color, colors }: { title: string; pct: number; color: string; colors: ReturnType<typeof import("@/hooks/useColors").useColors> }) {
  return (
    <View style={[styles.progressCard, { backgroundColor: colors.card, flex: 1 }]}>
      ...
      <Text style={[styles.progressSub, { color: colors.mutedForeground }]}>tamamlandÄ±</Text>
    </View>
  );
}

function getMotivationMessage(overallPct: number): { text: string; emoji: string } {
  if (overallPct === 0)   return { text: "Harika bir baÅŸlangÄ±Ã§ seni bekliyor!", emoji: "ðŸŒŸ" };
  if (overallPct < 10)    return { text: "Her bÃ¼yÃ¼k baÅŸarÄ± ilk konuyla baÅŸlar. Devam et!", emoji: "ðŸš€" };
  if (overallPct < 25)    return { text: "GÃ¼zel bir baÅŸlangÄ±Ã§! Her gÃ¼n biraz daha ilerliyorsun.", emoji: "ðŸ’ª" };
  if (overallPct < 40)    return { text: "Ã‡alÄ±ÅŸmalarÄ±n meyve vermeye baÅŸlÄ±yor. Harika gidiyorsun!", emoji: "ðŸ“ˆ" };
  if (overallPct < 55)    return { text: "YarÄ±ya yaklaÅŸtÄ±n! Her soru seni hedefe yaklaÅŸtÄ±rÄ±yor.", emoji: "ðŸŽ¯" };
  if (overallPct < 70)    return { text: "YarÄ±yÄ± geÃ§tin! Ã‡ok az kaldÄ±, dur durma!", emoji: "âš¡" };
  if (overallPct < 80)    return { text: "Harika ilerliyorsun! Hedef artÄ±k Ã§ok yakÄ±n.", emoji: "ðŸ”¥" };
  if (overallPct < 90)    return { text: "Neredeyse hazÄ±rsÄ±n! Son dÃ¼zlÃ¼ÄŸe girdin.", emoji: "ðŸ†" };
  if (overallPct < 100)   return { text: "Sadece birkaÃ§ konu kaldÄ±. Sen yapabilirsin!", emoji: "âœ¨" };
  return { text: "TÃ¼m konularÄ± tamamladÄ±n! ArtÄ±k tekrar ve denemeler zamanÄ±.", emoji: "ðŸŽ‰" };
}
    ...
    <View style={[styles.remainingCard, { backgroundColor: colors.card }]}>
      ...
      <View style={styles.remainingRow}>
        ...
        <View style={styles.remainingItem}>
          ...
          <Text style={[styles.remainingLabel, { color: colors.mutedForeground }]}>Ã‡Ã¶zÃ¼len Soru</Text>
        </View>
      </View>
...
export default function HomeScreen() {
  ...
  const todayWd = new Date().getDay(); // 0=Sun â€¦ 6=Sat
  ...
  const pendingSessions = useMemo(() => sessions.filter((s) => {
    ...
    if (s.repeatType === "every_day") {
      ...
      if (yesterday < s.date) return false; // Session was created today â€” nothing overdue yet
    ...
    if (s.repeatType === "every_week") {
      ...
      const todayDay = todayDate.getDay(); // 0=Sun â€¦ 6=Sat
      for (const wd of weekdays) {
        ...
        if (occStr < s.date) continue; // Occurrence predates session creation â€” skip
    ...
    >
      <Animated.View entering={FadeInDown.delay(0).duration(500)}>
        <View style={styles.header}>
          <View>
            ...
            <Text style={[styles.userName, { color: colors.foreground }]} numberOfLines={1} ellipsizeMode="tail">{profile?.name ?? "Ã–ÄŸrenci"}</Text>
          </View>
      ...
      <Animated.View entering={FadeInDown.delay(60).duration(500)}>
        <View style={[styles.quoteCard, { backgroundColor: colors.card }]}>
          ...
          <Text style={[styles.quoteAuthor, { color: colors.mutedForeground }]}>â€” {quote.author}</Text>
        </View>
      </Animated.View>
      ...
      <Animated.View entering={FadeInDown.delay(120).duration(500)}>
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>SÄ±nava Geri SayÄ±m</Text>
      ...
      <Animated.View entering={FadeInDown.delay(180).duration(500)}>
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Konu Ä°lerlemesi</Text>
      ...
      <Animated.View entering={FadeInDown.delay(260).duration(500)}>
        <View style={styles.statsRow}>
          ...
          <View style={[styles.statCard, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.statLabel, { color: colors.mutedForeground }]}>GÃ¼n Serisi</Text>
          </View>
          <View style={[styles.statCard, { backgroundColor: colors.card }]}>
      ...
      <Animated.View entering={FadeInDown.delay(300).duration(500)}>
        <View style={styles.sectionHeader}>
          <Text style={[styles.sectionTitle, { color: colors.foreground, marginBottom: 0 }]}>BugÃ¼nÃ¼n PlanÄ±</Text>
          <TouchableOpacity onPress={() => router.push("/(tabs)/plan")}>
            <Text style={[styles.seeAll, { color: colors.primary }]}>TÃ¼mÃ¼</Text>
          </TouchableOpacity>
        </View>
        ...
        {pendingSessions.length > 0 && (
          <View style={[styles.warningBanner, { backgroundColor: colors.warning + "15", borderColor: colors.warning + "30" }]}>
            ...
            <Text style={[styles.warningText, { color: colors.warning }]}>{pendingSessions.length} gecikmiÅŸ gÃ¶rev var</Text>
          </View>
        )}
        ...
        {todaySessions.length === 0 ? (
          <View style={[styles.emptyTask, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>BugÃ¼n iÃ§in plan yok</Text>
      ...
      <Animated.View entering={FadeInDown.delay(360).duration(500)}>
        <View style={styles.quickActions}>
          <TouchableOpacity onPress={() => router.push("/statistics")} style={[styles.quickAction, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.quickActionText, { color: colors.foreground }]}>Ä°statistikler</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity onPress={() => router.push("/achievements")} style={[styles.quickAction, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.quickActionText, { color: colors.foreground }]}>BaÅŸarÄ±lar</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => router.push("/ai-coach")} style={[styles.quickAction, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.quickActionText, { color: colors.foreground }]}>AI KoÃ§</Text>
          </TouchableOpacity>
        </View>
      </Animated.View>
    </ScrollView>
